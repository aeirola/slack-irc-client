...
var IRC_EVENTS = {
  registered: function registered(context, message) {
    ...
    if (context.irc.options.hasOwnProperty('nsPassword')) {
      context.logger.info('nsPassword in options, attempting Nickserv IDENTIFY in 5 seconds...');
      setTimeout(function() {
        context.irc.client.send('/msg Nickserv IDENTIFY '+context.irc.options.nsPassword);
      }, 5000);
    }
  },
  ...
  join: function join(context, channel, nick, message) {
    ...
    // Only send the JOIN to slack if it isn't ignored.
    if (context.irc.options.hasOwnProperty('mute') && !context.irc.options.mute.includes('join')) {
  ...
  part: function part(context, channel, nick, reason, message) {
    ...
    // Only send the PART to slack if it isn't ignored.
    if (context.irc.options.hasOwnProperty('mute') && !context.irc.options.mute.includes('part')) {
  ...
  quit: function quit(context, nick, reason, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      if (nick in context.irc.client.chans[channel].users) {
        // Only send the QUIT to slack if it isn't ignored.
        if (context.irc.options.hasOwnProperty('mute') && !context.irc.options.mute.includes('quit')) {
  ...
  kick: function kick(context, channel, nick, by, reason, message) {
    ...
    // Only send the KICK to slack if it isn't ignored.
    if (context.irc.options.hasOwnProperty('mute') && !context.irc.options.mute.includes('kick')) {
  ...
  kill: function kill(context, nick, reason, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      // see https://github.com/martynsmith/node-irc/pull/376
      if (nick in context.irc.client.chans[channel].users) {
        // Only send the KILL to slack if it isn't ignored.
        if (context.irc.options.hasOwnProperty('mute') && !context.irc.options.mute.includes('kill')) {
  ...
  nick: function nick(context, oldnick, newnick, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      if (newnick in context.irc.client.chans[channel].users) {
        // Only send the NICK to slack if it isn't ignored.
        if (context.irc.options.hasOwnProperty('mute') && !context.irc.options.mute.includes('nick')) {
