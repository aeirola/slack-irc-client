...
var SLACK_EVENTS = {
  ...
  message: function message(context, message) {
    ...
    if (!message.subtype) {
      ...
      context.logger.warn('Slack: Setting of topic not yet supported');
      // TODO: Check how to disable topic reset loop
  ...
  channel_joined: function channelJoined(context, message) {
    context.logger.verbose('Slack: Joining channel #%s', message.channel.name);
    context.irc.client.join('#' + message.channel.name);
  },
  channel_left: function channelLeft(context, message) {
    var channel = context.slack.client.data.getChannelById(message.channel);
    context.logger.verbose('Slack: Leaving channel #%s', channel.name);
    context.irc.client.part('#' + channel.name);
    // TODO: Don't act on part messages
  },
