...
var IRC_EVENTS = {
  ...
  names: function names(context, channel, nicks) {
    var nickNames = _.chain(Object.keys(nicks))
                     .map(HELPERS.mrkdwnNick.bind(null))
                     .valueOf().join(', ');
  ...
  nick: function nick(context, oldnick, newnick, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      if (newnick in context.irc.client.chans[channel].users) {
        HELPERS.sendChannelStatus(context, channel, HELPERS.mrkdwnNick(oldnick)
                                  + ' changed nick to ' +
                                  HELPERS.mrkdwnNick(newnick));
      }
    });
  },
