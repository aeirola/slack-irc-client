...
var HELPERS = {
  ...
  isNotMuted: function isNotMuted(context, mutedAction) {
    if (context.irc &&
        context.irc.hasOwnProperty('options') &&
        context.irc.options &&
        context.irc.options.hasOwnProperty('muteIrcEvents') &&
        context.irc.options.muteIrcEvents.indexOf(mutedAction) !== -1) {
      return false;
    }
    return true;
  }
...
var IRC_EVENTS = {
  ...
  part: function part(context, channel, nick, reason, message) {
    ...
    if (HELPERS.isNotMuted(context, 'part')) {
  ...
  quit: function quit(context, nick, reason, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      if (nick in context.irc.client.chans[channel].users) {
        ...
        if (HELPERS.isNotMuted(context, 'quit')) {
  ...
  kick: function kick(context, channel, nick, by, reason, message) {
    ...
    if (HELPERS.isNotMuted(context, 'kick')) {
  ...
  kill: function kill(context, nick, reason, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      if (nick in context.irc.client.chans[channel].users) {
        ...
        if (HELPERS.isNotMuted(context, 'kill')) {
  ...
  nick: function nick(context, oldnick, newnick, channels, message) {
    ...
    _.forEach(channels, function(channel) {
      ...
      if (newnick in context.irc.client.chans[channel].users) {
        ...
        if (HELPERS.isNotMuted(context, 'nick')) {
